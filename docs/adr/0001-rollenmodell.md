...
| **Status** | angenommen |
| **Datum** | 2026-08-19 |
| **Fortgeschrieben** | 2026-09-23 — Abschnitte 3 und 8: Zwei Entscheide des Auftraggebers über den zweiten Formweg (Sitzungsauftrag), eingetragen in Abschnitt 8: Wer `.claude/skills/` beschreiben darf — künftig der SecDevOps Engineer wie bei den Hooks; CLAUDE.md, die Regeln unter `.claude/rules/` und die Rollendateien sind nicht Gegenstand des Entscheids, die ersten beiden Skills bleiben unverändert; nachgeführt in Abschnitt 3 (Zeile SecDevOps Engineer). Die Prüfstand-Fälle des Prüfmittels der beiden PreToolUse-Gates (ZF9, ZF10a bis ZF10d, L01 bis L16) als Fortschreibung von ADR 0002, 6.13 c durch den Software Architect in dieser Session mit erledigen: ja — umgesetzt am 2026-09-23 (ADR 0002, 6.13 c). 2026-09-21 — Abschnitte 4, 7.4 und 8: Die förmliche Freigabe der Entscheidpunkte E-A bis E-K war bereits am 2026-09-07 durch Merge erteilt, Merge-Commit `135e3614197a8150ad3d96fbf32eb0e893c9cbbc` (ADR 0002, Abschnitt 10); die Abnahme des DoD-Gates aus R3-Q-001 ist am 2026-09-08 durch Merge erteilt (ADR 0002, Abschnitt 10); Teil 2 des Abnahmekriteriums war nicht erfüllt, getragen hat der in ADR 0002, 6.12.28 g vorab festgelegte Weg; nicht umfasst sind O-25, der Belegprüfer D20 (O-15), R3-Q-005 und die Freigabe des Grundgerüsts; R3-Q-005 bleibt unverändert offen, die Lücke aus E-K bleibt. 2026-09-02 — Abschnitte 4, 7.4 und 8: Die Obergrenze der aufeinanderfolgenden Blockaden ist neu ohne den nicht belegten Variablennamen `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP` formuliert; belegt ist allein die Obergrenze von acht aufeinanderfolgenden Blockaden (ADR 0002, O-19). Die Gates für `Stop`, `SubagentStop` und `TaskCompleted` sind am 2026-09-02 nach ADR 0002, Abschnitt 6.12, auf Weisung des Auftraggebers gebaut; förmliche Freigabe und Abnahme stehen aus (ADR 0002, Abschnitt 10). R3-Q-005 bleibt offen und ist von diesem Gate nicht gedeckt; die Reichweite des Gates gegenüber Rollen ohne Schreibwerkzeug ist samt der Lücke bei `Bash` aufgenommen (ADR 0002, 6.12.14, Entscheidpunkt E-K). 2026-08-31 — Abschnitte 3, 5.1, 5.5 und 8: Die ersten beiden Skills liegen vor, das `skills:`-Feld ist bei neun Rollen gesetzt (R3-C-007 insoweit erfüllt); die Begründung der Rollentrennung in Abschnitt 1 stützte sich auf Felder, die es unterdessen auch für Skills gibt, und ist durch eine tragfähige in 5.5 ersetzt; neun Rollen führen `Skill` in ihrer Werkzeugliste, solange die Wirkung des Vorladens unbelegt ist — als zweiter Weg, nicht als Voraussetzung des Vorladens, und terminiert; die Wirkung des Vorladens ist in der laufenden Sitzung nicht feststellbar und zu Beginn der nächsten zu prüfen; neuer offener Punkt: wer `.claude/skills/` beschreiben darf. 2026-08-26 — Abschnitte 2.4, 3 und 5.2: `maxTurns` des Product Owners von 25 auf 50, nach gemessenem Abbruch an der Grenze (Begruendung in 5.2). 2026-08-25 — Abschnitte 2.4, 3 und 5.2: `maxTurns` des Static Software Testers von 30 auf 80, nach gemessenem Abbruch an der Grenze (Begründung in 5.2). 2026-08-20 — Abschnitte 4, 7.4 und 8: Stand der `settings.json`; Terminierung der Hooks (R3-Q-001 aus Schritt 3, R3-Q-005 ergänzt am 2026-08-20); Auflösung der Befunde V-02 und V-04 (`docs/08_Freigabe_Schritt_4.md`) |
...
| SecDevOps Engineer | 4.2 | `secdevops-engineer.md` | sonnet | 30 | Read, Grep, Glob, Edit, Write, Bash, Skill | ja | Pipeline- und Hook-Konfiguration, SBOM, Secret-Scanning; `Bash`, weil die Wirkung über den Rückgabewert belegt wird (3.4). Seit dem 2026-09-23 auch die Skills unter `.claude/skills/` (Entscheid des Auftraggebers, Abschnitt 8); die ersten beiden Skills hat der Koordinator angelegt. |
...
**Nachtrag vom 2026-08-31 — `Skill` bei neun Rollen, und weshalb die erste Begründung dafür falsch war.** Die neun Rollen, denen in 5.1 ein Skill zugeordnet ist, führen `Skill` neu in ihrer Werkzeugliste.

Die erste Fassung dieses Nachtrags begründete das damit, eine Positivliste ohne `Skill` nehme der Rolle die Fähigkeit, eine zugeordnete Prozedur überhaupt zu laden. **Das ist falsch, und die Nachprüfung vom 2026-08-31 hat es an der Dokumentation belegt**, die dieser Abschnitt selbst als Grundlage nennt: Das Vorladen über `skills:` legt den Inhalt beim Start in den Kontext des Subagenten, **unabhängig vom `tools`-Feld** — die Dokumentation rät sogar ausdrücklich, zum Vorladen `skills:` zu benutzen statt `Skill` in `tools` aufzuführen. Was `Skill` in `tools` steuert, ist etwas anderes: ob der Subagent während der Ausführung **weitere, nicht vorgeladene** Skills finden und aufrufen kann. Quelle: https://code.claude.com/docs/en/sub-agents.md, Abschnitt "Preload skills into subagents", geprüft am 2026-08-31.

**Der tragende Grund ist ein anderer und steht hier neu.** Die Wirkung des Vorladens ist in dieser Umgebung nicht festgestellt (5.1). Solange das so ist, ist `Skill` in der Werkzeugliste der Weg, auf dem die Rolle die zugeordnete Prozedur mit Sicherheit erreicht — und zugleich der Weg zu einer Prozedur, die ihr nicht vorgeladen ist. Die Erweiterung ist eng: `Skill` liest eine Prozedur aus dem eigenen Repository, es gewährt weder Datei- noch Shell-Zugriff und ändert an den Schreibrechten nichts. Die zwölf Rollen ohne Skill-Zuordnung erhalten das Werkzeug nicht.

**Und sie ist terminiert.** Ergibt die Prüfung zu Beginn der nächsten Sitzung (5.1), dass das Vorladen wirkt, wird `Skill` in den neun Werkzeuglisten erneut begründet oder entfernt. Ein Werkzeug, dessen Grund entfallen ist, bleibt nicht aus Gewohnheit stehen — das wäre das Gegenteil des Prinzips der minimalen Rechte.
...
Für alle drei gilt der in 3.4 als häufigster Fehler benannte Punkt: **Nur Rückgabewert 2 blockiert.** Ein Gate mit `exit 1` ist wirkungslos, ohne dass das auffällt. Ebenso verbindlich ist der Reentranz-Schutz über `stop_hook_active`. Dazu kommt eine Obergrenze: Nach **acht aufeinanderfolgenden Blockaden** übergeht Claude Code den Hook und beendet den Turn. Ein Gate ist so zu bauen, dass es diese Grenze nicht braucht.

*Berichtigt am 2026-09-02 (ADR 0002, O-19).* Hier stand, ebenso verbindlich sei "die harte Obergrenze über `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`". **Dieser Variablenname ist nicht belegt.** In der am 2026-09-02 gelesenen offiziellen Hook-Dokumentation kommt er nicht vor; belegt ist dort allein die Obergrenze ohne Namen — "Claude Code overrides the hook and ends the turn after 8 consecutive blocks". Der Absatz sagt deshalb neu nur noch das Belegte: dass es eine Obergrenze von acht aufeinanderfolgenden Blockaden gibt. Ob sie sich über eine Umgebungsvariable verstellen lässt und wie diese heisst, behauptet dieser ADR nicht mehr — eine verbindlich erklärte Einstellung, die es möglicherweise gar nicht gibt, ist als Bauvorschrift untauglich. **Die Verbindlichkeit des Reentranz-Schutzes bleibt unberührt.** Das am 2026-09-02 gebaute Gate stützt sich an keiner Stelle auf die Variable und braucht sie auch nicht: Mit dem Reentranz-Schutz ist eine Folge von acht Blockaden innerhalb eines Beendigungsversuchs ohnehin unerreichbar (ADR 0002, 6.12.2). Die gleichlautenden Stellen in Projektauftrag 3.4 und in `docs/06_Definition_of_Ready_und_Done.md` ändert diese Fortschreibung nicht; sie sind als O-19 dem Requirements Engineer mit dem Protocol Master zugewiesen, der Entscheid über eine Änderung des Projektauftrags liegt beim Auftraggeber.
...
**Fortgeschrieben am 2026-09-02 — die Prüfketten-Gates bestehen, die Verzeichnisbeschränkung weiterhin nicht.** Die Überschrift des vorstehenden Absatzes ist damit zur Hälfte überholt; sie bleibt als Stand vom 2026-08-20 stehen, weil Historie nicht umgeschrieben wird. Gebaut sind am 2026-09-02 nach ADR 0002, Abschnitt 6.12, auf Weisung des Auftraggebers vom selben Tag: das Hook-Skript `.claude/hooks/dod-gate.sh`, die versionierte Liste der terminierten Lagen C `.claude/hooks/dod-gate-terminierte-lagen.txt` und **drei Einträge in `.claude/settings.json`** für `Stop`, `SubagentStop` und `TaskCompleted`. Das Gate lässt eine Antwort, einen Subagenten und eine Aufgabe erst enden, wenn `make dod` im geprüften Arbeitsbaum nachweisbar gelaufen ist und nichts gefunden hat. **R3-Q-001 ist damit gebaut, aber nicht abgeschlossen:** Die förmliche Freigabe der elf Entscheidpunkte E-A bis E-K und die Abnahme stehen aus (ADR 0002, Abschnitt 10, "Vorlage der zwölften Fortschreibung und Weisung vom 2026-09-02"); die Verifikation liegt beim Static und beim Dynamic Software Tester (3.4), nicht bei der Rolle, die den ADR schreibt. *Fortgeschrieben am 2026-09-21:* Die förmliche Freigabe der Entscheidpunkte E-A bis E-K war bereits am 2026-09-07 durch Merge erteilt, Merge-Commit `135e3614197a8150ad3d96fbf32eb0e893c9cbbc` (ADR 0002, Abschnitt 10). Die Abnahme des DoD-Gates aus R3-Q-001 ist am 2026-09-08 durch Merge des Pull Requests #15 erteilt, Merge-Commit `9870b0d115b8ef330a7c19777af5741093e4f0e9`, ohne Auflagen (ADR 0002, Abschnitt 10); Teil 2 des Abnahmekriteriums nicht erfüllt, getragen hat der in ADR 0002, 6.12.28 g vorab festgelegte Weg; nicht umfasst sind O-25, der Belegprüfer D20 (O-15), R3-Q-005 und die Freigabe des Grundgerüsts. R3-Q-005 bleibt offen; die Verzeichnisbeschränkung besteht weiterhin nicht.

**R3-Q-005 bleibt offen, und dieses Gate deckt es nicht.** Die harte Durchsetzung der Rollen-Schreibgrenzen über `PreToolUse` ist nicht gebaut; es besteht damit weiterhin **keine technische Verzeichnisbeschränkung für irgendeine Rolle**. Das neue Gate ersetzt sie nicht und gibt nicht vor, es zu tun: Es misst nach ADR 0002, 6.12.14, über das `tools`-Feld das **Recht** einer Rolle, den Arbeitsbaum zu ändern, nicht ihre **Fähigkeit** dazu. Für die dreizehn Rollen mit abgestufter Rechteform aus der Tabelle oben bleibt die weiche Form deshalb die einzige.

**Reichweite von `SubagentStop`: Rollen ohne Schreibwerkzeug prüft das Gate nicht** (ADR 0002, 6.12.14, Entscheidpunkt E-K). Beim Beenden eines Subagenten bestimmt das Gate zuerst dessen Rolle — über das Frontmatter-Feld `name:` der Dateien unter `.claude/agents/`, nicht über den Dateinamen — und liest deren `tools`-Feld. Enthält es **keines von `Edit`, `Write` und `NotebookEdit`**, läuft die Prüfkette für diese Rolle gar nicht erst; das Gate endet mit 0 und meldet, dass und weshalb es nicht geprüft hat. Betroffen sind nach der Rollentabelle in Abschnitt 3 der **Static Software Tester** und der **Pentester**, die beiden Rollen mit "nein" in der Spalte "Schreibrechte laut 4.2". Der Grund ist ihr Auftrag: Ihre Aufgabe ist es, den roten Befund zu melden; ein Gate, das sie wegen desselben roten Befunds am Abliefern hindert, macht die Meldung unmöglich. Lässt sich eine Rolle nicht eindeutig auflösen, gilt das Gegenteil — sie wird als schreibberechtigt behandelt, und die Kette läuft.

**Die dazugehörige Lücke, benannt statt überdeckt.** `Bash` zählt in diesem Kriterium **nicht** als Schreibwerkzeug, weil dieser ADR Schreibrecht über `Edit`, `Write` und `NotebookEdit` vergibt und das Kriterium mit `Bash` ausgerechnet bei den beiden Rollen liefe, für die es gemacht ist. Eine Rolle mit `Bash`, aber ohne diese drei Werkzeuge — Static Software Tester und Pentester tragen genau diese Kombination — **könnte** über die Shell schreiben, mit einer Umleitung, mit `sed -i` oder über einen Interpreteraufruf; nach diesem ADR **darf** sie es nicht (Abschnitt 2.1 und die Rollentabelle in Abschnitt 3). Gemessen wird also das Recht, nicht die Fähigkeit. Solange R3-Q-005 nicht steht, ist der Fall "Prüfrolle schreibt über `Bash`" durch nichts abgefangen. Das ist eine benannte Lücke des Rollenmodells und keine, die das Gate einführt.
...
1. **Zweiter Weg gebaut.** Die neun Rollen führen `Skill` in ihrer Werkzeugliste (Nachtrag zu Abschnitt 3). Damit erreicht eine Rolle die Prozedur auch dann, wenn das Vorladen nicht wirkt. **Nicht** deshalb, weil das Vorladen dieses Werkzeug bräuchte — das tut es nach der Dokumentation nicht —, sondern weil ein zweiter Weg trägt, solange der erste unbelegt ist.
...
**Beobachtung am Rand, 2026-08-26 — eine Änderung an `maxTurns` wirkt nicht in der laufenden Sitzung.** Der Wert wurde auf 50 gesetzt, *bevor* der Product Owner erneut beauftragt wurde; der Lauf stoppte gleichwohl bei 25 Zügen. Das ist ein Einzelbeleg, kein Beweis, aber er deutet darauf hin, dass die Rollendateien beim Sitzungsstart gelesen und danach nicht erneut eingelesen werden. **Folge für die Praxis:** Wer `maxTurns` korrigiert, darf nicht erwarten, dass die Korrektur die gerade laufende Arbeitseinheit rettet — sie greift ab der nächsten Sitzung. Das passt zu der in diesem Abschnitt bereits festgehaltenen Lücke: Die Dokumentation sagt nicht, wann das Frontmatter gelesen wird. Bei der nächsten Gelegenheit gegenzuprüfen; bis dahin ist es als Beobachtung und nicht als Festlegung zu lesen.

Der Fall bestätigt die Erwartung, dass die Werte Startwerte sind und korrigiert werden, ein zweites Mal und zeigt zugleich die Grenze der ursprünglichen Staffelung: Sie bemass den Wert am Umfang des *Arbeitsprodukts* der Rolle. Der Product Owner verantwortet mit dem Backlog ein eng umrissenes Arbeitsprodukt, doch eine Einordnung, die jede Zahl aus dem Dateiinhalt nachrechnet, statt sie fortzuschreiben, ist unabhängig davon aufwendig. Massgeblich ist nicht die Breite des Arbeitsprodukts, sondern die Zahl der Prüfschritte, die eine Arbeitseinheit verlangt.
...
*Fortgeschrieben am 2026-09-02:* Von den beiden Terminierungen ist eine eingelöst. **R3-Q-001 ist gebaut** — `Stop`, `SubagentStop` und `TaskCompleted` sind am 2026-09-02 nach ADR 0002, Abschnitt 6.12, auf Weisung des Auftraggebers als `.claude/hooks/dod-gate.sh` mit `.claude/hooks/dod-gate-terminierte-lagen.txt` und drei Einträgen in der versionierten `.claude/settings.json` umgesetzt; förmliche Freigabe der Entscheidpunkte E-A bis E-K und Abnahme stehen aus (ADR 0002, Abschnitt 10). **R3-Q-005 bleibt offen und ist von diesem Gate nicht gedeckt:** Es misst das Recht einer Rolle über ihr `tools`-Feld, nicht ihre Fähigkeit zu schreiben (ADR 0002, 6.12.14). Die Rechtestaffelung der dreizehn Rollen mit abgestufter Rechteform bleibt damit eine Verhaltensvorgabe; Entscheid E-02 bleibt für R3-Q-005 unverändert offen. Einzelheiten in Abschnitt 4. *Fortgeschrieben am 2026-09-21:* Die förmliche Freigabe der Entscheidpunkte E-A bis E-K war bereits am 2026-09-07 durch Merge erteilt, Merge-Commit `135e3614197a8150ad3d96fbf32eb0e893c9cbbc` (ADR 0002, Abschnitt 10). R3-Q-001 ist abgenommen; die drei Restlücken und die nachrangigen Befunde der Runde 12 bleiben als Posten unter R3-Q-001 im Backlog offen — Abnahme am 2026-09-08 durch Merge erteilt (Merge-Commit `9870b0d115b8ef330a7c19777af5741093e4f0e9`, ADR 0002, Abschnitt 10; Teil 2 des Abnahmekriteriums nicht erfüllt, getragen hat der in ADR 0002, 6.12.28 g vorab festgelegte Weg). R3-Q-005 und Entscheid E-02 bleiben unverändert offen; die Lücke aus E-K bleibt.
...
| Hooks in der versionierten `.claude/settings.json` — **zur Hälfte erledigt am 2026-09-02:** R3-Q-001 ist gebaut (`Stop`, `SubagentStop`, `TaskCompleted`), förmliche Freigabe und Abnahme stehen aus; **R3-Q-005 bleibt offen** und ist vom neuen Gate nicht gedeckt (Abschnitt 4, ADR 0002, 6.12.14). **Fortgeschrieben am 2026-09-21:** Die förmliche Freigabe der Entscheidpunkte E-A bis E-K war bereits am 2026-09-07 durch Merge erteilt, Merge-Commit `135e3614197a8150ad3d96fbf32eb0e893c9cbbc` (ADR 0002, Abschnitt 10); Abnahme von R3-Q-001 am 2026-09-08 erteilt (ADR 0002, Abschnitt 10); im Rahmen dieser Zeile offen bleibt allein R3-Q-005; O-25 und die Abnahme des Belegprüfers D20 (O-15) bleiben davon unberührt offen; Teil 2 des Abnahmekriteriums war nicht erfüllt (ADR 0002, 6.12.28 g) | 3.4; terminiert als R3-Q-001 und R3-Q-005 in Etappe 0 (Fortschreibung 2026-08-20); Bau von R3-Q-001 nach ADR 0002, 6.12, Freigabevermerk in ADR 0002, Abschnitt 10 | Folgearbeit im Backlog; Freigabe und Abnahme beim Auftraggeber, Verifikation bei Static und Dynamic Software Tester (3.4) |
...
| **Wer `.claude/skills/` beschreiben darf.** Abschnitt 4 weist `.claude/` keiner Rolle zu: Der Protocol Master schreibt nur `docs/`, die Hook-Konfiguration liegt beim SecDevOps Engineer. Die ersten beiden Skills hat der Koordinator angelegt, wie zuvor `CLAUDE.md`, die Regeln und die Hooks in Lieferschritt 2. Vor der dritten Skill ist zu entscheiden, ob das so bleibt. **Entschieden am 2026-09-23** durch den Auftraggeber über den zweiten Formweg, Wortlaut: "Wer .claude/skills/ beschreiben darf: [A] Koordinator wie bisher oder [B] SecDevOps Engineer wie bei den Hooks. ICH WÄHLE: [B]". Der SecDevOps Engineer schreibt die Skills unter `.claude/skills/`, wie die Hooks; Verifikation durch den Static Software Tester auf einem anderen Modell (3.4). Nicht Gegenstand des Entscheids: CLAUDE.md, die Regeln unter `.claude/rules/` und die Rollendateien; die ersten beiden Skills bleiben, wie sie sind. Nachgeführt am 2026-09-23: Abschnitt 3 (Zeile SecDevOps Engineer), `.claude/agents/secdevops-engineer.md` und `.claude/rules/claude-konfiguration.md`, Abschnitt "Skills" (nachgeführt durch den Koordinator) | **Neu am 2026-08-31** | Auftraggeber — entschieden am 2026-09-23 |
| **Prüfstand-Fälle des Prüfmittels der beiden PreToolUse-Gates** (ZF9, ZF10a bis ZF10d, L01 bis L16) — **entschieden am 2026-09-23** durch den Auftraggeber über den zweiten Formweg, Wortlaut: "Prüfstand-Fälle des Prüfmittels (ZF9, ZF10a bis ZF10d, L01 bis L16) als Fortschreibung von ADR 0002, 6.13 c durch den Software Architect in dieser Session mit erledigen: ja/nein. ICH WÄHLE: ja"; umgesetzt am 2026-09-23 als Nachtrag zu ADR 0002, 6.13 c (L01 bis L09 als belegte Lücken in drei Posten unter R3-Q-010, ZF10a, ZF10b, ZF10d und L10 bis L16 als Grenze, ZF9 und ZF10c als gewolltes Verhalten). Gegenstand ist ADR 0002, nicht dieser ADR; hier eingetragen, weil der Sitzungsauftrag beide Entscheide in diesem Abschnitt verlangt | Sitzungsauftrag vom 2026-09-23; ADR 0002, 6.13 c | entschieden durch den Auftraggeber am 2026-09-23; Umsetzung Software Architect |
