...
if [ "$tool" = "Bash" ]; then
  ...
  # Einen zweiten Arbeitsbaum auf main/master anzulegen ist die Wurzel der
  # Worktree-Umgehung: danach schreiben 'git -C <wt> commit' oder 'sed -i
  # <wt>/datei' an main vorbei am Zweig-Bezug dieses Gates (Befund der
  # statischen Pruefung vom 2026-08-25). Deshalb schon das Anlegen sperren.
  if printf '%s' "$cmd" | grep -Eq 'git[[:space:]]+([^|;&]*[[:space:]])?worktree[[:space:]]+add([[:space:]]|$)' \
    ...
    echo "Ein zweiter Arbeitsbaum auf main umgeht den Schutz. Einen Arbeitszweig auschecken." >&2
    exit 2
  fi

  # Zielzweig bestimmen: bei 'git -C <pfad>' der Zweig jenes Pfads, sonst der
  # des Projektbaums. So greift die git-Verb-Pruefung auch fuer einen bereits
  # bestehenden Arbeitsbaum, der auf main steht.
  ziel_branch="$branch"
  if printf '%s' "$cmd" | grep -Eq 'git[[:space:]]+-C[[:space:]]'; then
    cpath=$(printf '%s' "$cmd" | sed -nE 's/.*git[[:space:]]+-C[[:space:]]+("([^"]+)"|'\''([^'\'']+)'\''|([^[:space:]]+)).*/\2\3\4/p' | head -n 1)
    if [ -n "$cpath" ]; then
      b2=$(git -C "$cpath" rev-parse --abbrev-ref HEAD 2>/dev/null || echo "")
      [ -n "$b2" ] && ziel_branch="$b2"
    fi
  fi
  ...
  # vorgesehene Ausweg von main herunter. Geprueft wird der Zielzweig (oben),
  # damit auch 'git -C <main-worktree> commit' greift.
  if is_protected "$ziel_branch" && printf '%s' "$cmd" | grep -Eq 'git[[:space:]]+([^|;&]*[[:space:]])?(commit|merge|revert|cherry-pick|rebase|reset|am|apply|stash[[:space:]]+(pop|apply))([[:space:]]|$)'; then
    echo "BLOCKIERT (Projektauftrag 3.2 c): schreibender git-Befehl auf '$ziel_branch'." >&2
    echo "Zuerst einen Arbeitszweig anlegen: git switch -c <zweig>" >&2
    exit 2
  fi
  ...
  # ein Schreib-Schluesselwort im Text steht (Befund 2026-08-25). Auf main
  # werden sie deshalb pauschal blockiert -- eine bewusste Ueberdeckung: auf
  # main wird ohnehin nicht gearbeitet, und der Ausweg ist ein Arbeitszweig.
  # Vollstaendig ist die Textpruefung hier nicht (siehe Kopf); die harte
  # Zusicherung liegt beim serverseitigen Ruleset.
  if is_protected "$branch"; then
    ...
    if printf '%s' "$bereinigt" | grep -Eq '(>>?|\btee[[:space:]]|sed[[:space:]]+-[a-zA-Z]*i|\b(mv|cp|rm|mkdir|touch|truncate|ln|install|patch|dd)[[:space:]]|\b(python[0-9.]*|perl|ruby|node|deno|php|Rscript)[[:space:]]+([^|;&]*[[:space:]]+)?-[a-zA-Z]*(c|e|i|p|r|n)([^a-zA-Z]|$)|\b(ed|ex)[[:space:]])'; then
      echo "BLOCKIERT (Projektauftrag 3.2 c): Shell-Befehl mit moeglicher Schreibwirkung, waehrend HEAD auf '$branch' steht." >&2
