...
- Kanonische Standards: FollowTheMoney, STIX 2.1, W3C PROV; Datenhaltung PostgreSQL. `pgvector` ist nicht Bestandteil des Aufbaus — entschieden in ADR 0002 (A4): keine Erweiterung, keine Spalte, keine Migration, kein Platzhalter. Eine spätere Aufnahme braucht einen Backlog-Eintrag und einen eigenen ADR mit den Nachweisen aus A4 (5.18).
...
- Je Entscheidung ein Architecture Decision Record unter `docs/adr/`, mit Datum, Kontext, Optionen, Entscheid, Begründung und Konsequenzen — als Nächstes für die Komponentenbibliothek des Frontends (offener Punkt O-2 aus ADR 0002; das Rahmenwerk ist dort mit A8 entschieden, `pgvector` mit A4). Diese Rolle legt die Datei an, der Protocol Master führt sie in Changelog und Nachweisverzeichnis nach (4.2, 6.6).
