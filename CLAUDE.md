...
| `dod-gate.sh` | Lässt eine Antwort (`Stop`), einen Subagenten (`SubagentStop`) und eine Aufgabe (`TaskCompleted`) erst enden, wenn `make dod` im geprüften Arbeitsbaum nachweisbar gelaufen ist und nichts gefunden hat. Lagen C, die in `dod-gate-terminierte-lagen.txt` mit Grund eingetragen sind, werden mit Meldung geduldet; die Liste prüft sich selbst. Dreimaliges Scheitern am gleichen Kriterium verlangt die Übergabedatei (3.4). Rollen ohne `Edit`, `Write` oder `NotebookEdit` prüft es nicht (ADR 0002, 6.12; gebaut 2026-09-02 auf Weisung; acht Prüfrunden, zuletzt am 2026-09-04 nach 3.4 abgebrochen, O-26 vorgelegt, förmliche Freigabe und Abnahme ausstehend; Selbsttest `scripts/dod-gate-selbsttest.sh` mit 154 Zusicherungen `Z-nnn`, Deckung und Kanal mechanisch gegen ADR 0002, 6.12.19 abgeglichen, Mutationsprobe `scripts/dod-gate-mutationen.txt` über `--mutationen`; offene Befunde in `docs/uebergaben/2026-09-04_r3-q-001-o-25-kanal-und-mutation.md`) |
...
| DoD-Gate, terminierte Lagen C, Selbsttest, Mutationsprobe | `.claude/hooks/dod-gate.sh`, `.claude/hooks/dod-gate-terminierte-lagen.txt`, `scripts/dod-gate-selbsttest.sh`, `scripts/dod-gate-mutationen.txt`; Entwurf und Nachträge in ADR 0002, 6.12 |
