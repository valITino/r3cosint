...
| Gate | Wirkung |
|---|---|
| `block-prototype-import.sh` | Blockiert Importe zwischen `prototype/` und Produktionscode in beide Richtungen (5.6). Erkennt seit E4.2 (2026-09-23) auch den Verzeichnisimport ohne Schrägstrich, den Rückstrich-Pfad, den Präfix `./` sowie HTML- und Python-Formen in Richtung 2; die Meldung nennt 5.6 und die Richtung. Seit E4.3 (2026-09-23) auch die Schreibwirkung eines Interpreters mit Inline-Code (dieselbe Befehlsklasse wie das main-Gate) und eine nicht auswertbare Eingabe (Rückgabewert 2); die Nichtprüfung gemeinsamer Abhängigkeiten ist als Grenze benannt. Prüfmittel: `scripts/pretooluse-gates-selbsttest.sh` (R3-Q-010; keine belegte Lücke verbleibt, Prüfstand-Fälle mit heutigem Wert) |
| `block-main-write.sh` | Blockiert Dateiänderungen auf `main` sowie Commit, Merge und Push nach `main` (3.2 c). Seit E4.3 (2026-09-23) blockiert es auch eine nicht auswertbare Eingabe (Rückgabewert 2, ADR 0002, 6.13 b); Grenze bleibt der untergeordnete Shell-Aufruf, eine Subshell in Klammern wird erkannt. Prüfmittel: `scripts/pretooluse-gates-selbsttest.sh` (R3-Q-010; ST-01 bis ST-03 fremdbelegt) |
