...
| `dod-gate.sh` | Lässt eine Antwort (`Stop`), einen Subagenten (`SubagentStop`) und eine Aufgabe (`TaskCompleted`) erst enden, wenn `make dod` im geprüften Arbeitsbaum nachweisbar gelaufen ist und nichts gefunden hat. Lagen C, die in `dod-gate-terminierte-lagen.txt` mit Grund eingetragen sind, werden mit Meldung geduldet; die Liste prüft sich selbst. Dreimaliges Scheitern am gleichen Kriterium verlangt die Übergabedatei (3.4). Rollen ohne `Edit`, `Write` oder `NotebookEdit` prüft es nicht (ADR 0002, 6.12; gebaut 2026-09-02 auf Weisung; fünf Prüfrunden, zuletzt am 2026-09-03 nach 3.4 abgebrochen, O-25 vorgelegt, förmliche Freigabe und Abnahme ausstehend; Selbsttest `scripts/dod-gate-selbsttest.sh`, 152 Zusicherungen `Z-nnn` mit mechanischer Deckung gegen ADR 0002, 6.12.19; offene Befunde in `docs/uebergaben/2026-09-03_r3-q-001-o-24-zusicherungen.md`) |
